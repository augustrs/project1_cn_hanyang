...
int main(int argc, char *argv[]) {
  ...
  if (port <= 1024 || port > 65535) {
    printf(
        stderr,
        "Invalid port number. Use a port between 1025 and 65535.\n"); // error
                                                                      // message
  ...
  if (start_listening(server_socket, 5) <
      0) { // queue backlog of 5, so we do not have to wait for a connection
  ...
  while (1) {
    ...
    if (client_socket < 0) {
      printf(stderr, "Failed to accept client connection.\n");
      continue;
    }

    handle_client(
        client_socket); // handle client function handles the received message
...
int start_listening(int server_socket, int backlog) {
  if (listen(server_socket, backlog) < 0) { // if listen fails
...
int accept_connection(int server_socket) {
  ...
  int client_socket = accept(server_socket, (struct sockaddr *)&client_address,
                             &client_addrlen); // accept connection
  if (client_socket < 0) {                     // if accept fails
...
void handle_client(int client_socket) {
  ...
  ssize_t bytes_read = recv(client_socket, buffer, sizeof(buffer) - 1,
                            0);            // receive message from client
  if (bytes_read < 0) {                    // if recv fails
  ...
  const char *response =
      ...
      "html><html><head><title>Hey you!</title></head><body><h1>Hey "
      "you!</h1></body></html>"; // response message simple html page
